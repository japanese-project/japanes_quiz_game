...
>
	<div class="mx-auto flex w-full max-w-[1280px] items-center justify-between px-5 py-5 sm:px-8">
		...
		<div class="h-1.5 w-full overflow-hidden bg-white/10">
			<div
				class="h-full bg-[#e52f46] transition-all duration-500"
			...
			<div class="mx-auto flex w-full max-w-4xl flex-1 flex-col px-5 py-7 sm:px-8 sm:py-10">
				...
				<div class="mt-7 grid gap-3 sm:grid-cols-2">
					{#each current.choices as choice, choiceIndex (`${current.id}-${choiceIndex}`)}
						{@const isCorrect = submitted && choiceIndex === current.answer}
						{@const isWrong =
							submitted && selected === choiceIndex && choiceIndex !== current.answer}
						<button
						...
						>
							<span
								...
								>{String.fromCharCode(65 + choiceIndex)}</span
							>
							{choice}
							{#if isCorrect}<span class="ml-auto text-emerald-600"
									><Icon name="check" size={19} /></span
								>{:else if isWrong}<span class="ml-auto text-red-500"
