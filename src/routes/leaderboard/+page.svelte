...
{:else if leaderboard.error}
	...
	>
		<div>
			...
			<p class="type-body mt-2 text-text-secondary">Please check your connection and try again.</p>
			<button
				onclick={() => leaderboard.reconnect()}
				class="type-action mt-6 cursor-pointer rounded-full bg-primary px-5 py-3 font-bold text-text-primary transition hover:bg-primary-hover"
				>Reconnect</button
			>
		</div>
	</main>
